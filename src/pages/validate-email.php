...
<head>
	...
	<title>PHP Auth Flo | Validate</title>
...
<body>
	<?php
		if(isset($_GET['id']) && $_GET['id'] !== '' && isset($_GET['hash']) && $_GET['hash'] !== '') {
			...
			$C = connect();
			if($C) {
				$res = sqlSelect($C, 'SELECT user,hash,timestamp FROM requests WHERE id=? AND type=0', [$_GET['id']]);
				if($res) {
					$request = $res;
			...
			<div class="formWrapper">
				<form id="validate-email-form">
					...
					<br>
					<br>
					<a href="./login.php">Already Verified? Log In</a>
				</form>
